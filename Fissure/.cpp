...
extern "C"
{
  ...
  const char*  InNames[]  = {" Slot In",   nullptr};            // This array contains the names of each input slot as a string that can be used by the GUI.  It will show up as a label to each slot and be used when visualizing.
  const char* OutTypes[]  = {"IdxVerts",           nullptr};            // This array contains the types that are output in each slot of the same index
  const char* OutNames[]  = {" Slot Out",  nullptr};            // This array contains the names of each output slot as a string that can be used by the GUI.  It will show up as a label to each slot and be used when visualizing.

  void _construct(){ }
  void _destruct(){ }

  uint64_t (LavaParams const* lp, LavaFrame const* in, lava_threadQ* out) noexcept
  {
    using namespace std;
  ...
  LavaNode LavaNodes[] =
  {
    {
      ,                                      // function
      _construct,                            // constructor - this can be set to nullptr if not needed
      _destruct,                             // destructor  - this can also be set to nullptr 
