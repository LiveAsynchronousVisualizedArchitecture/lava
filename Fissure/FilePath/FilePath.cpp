...
extern "C"
{
  ...
  const char*  InNames[]  = {nullptr};                               // This array contains the names of each input slot as a string that can be used by the GUI.  It will show up as a label to each slot and be used when visualizing.
  ...
  uint64_t FilePath(LavaParams const* lp, LavaFrame const* in, lava_threadQ* out) noexcept
  {
    ...
    TO(pth.size(),i){ filePath.push( pth[i] ); }
