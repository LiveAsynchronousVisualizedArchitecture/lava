...
// -todo: debug why tbl::push() ruins the heap alloction - possibly because the stride is not being set correctly - stride was not being set correctly and there was no allocation to support setting the stride - fixed with a templated function to set the type
...
//      -| make cache node type - is there a reason to make a more general generator node type? - not clear, so cache type will remain - maybe a cache node that repeats a single packet and a flow node with no inputs is enough - what about a node that is pulling external data but needs settings changed with an input? - that input could be supplied by a cache, a flow node without inputs, or a constant, since the constant will also produce a packet on every cycle
//      -| how does a cache node clear its memory? - being sent a packet with a nullptr could clear its memory
//      -| make sure cache nodes owns their memory and produce allocations that make room for references - converted to use LavaMem
//      -| should a cache node be drawn with two sides slanting inwards from top to bottom? - different color? grey? - just set to a different color for now
