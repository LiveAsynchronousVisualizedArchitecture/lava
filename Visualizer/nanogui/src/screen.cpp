...
#if defined(_WIN32) || defined(__linux__)
    //mSize = Vector2i(mSize.cast<float>() / mPixelRatio).cast<int>();
    //mFBSize = Vector2i(mSize.cast<float>() * mPixelRatio).cast<int>();
    ...
    if (elapsed > 0.5f) {
        ...
        if (widget && !widget->tooltip().empty()) {
            ...
            if (h > tooltipWidth / 2) {
                ...
                nvgTextBoxBounds(mNVGContext, (float)pos.x(), (float)pos.y(), (float)tooltipWidth,
                                widget->tooltip().c_str(), nullptr, bounds);
            ...
            nvgMoveTo(mNVGContext, (float)px, bounds[1] - 10);
            nvgLineTo(mNVGContext, px + 7.f, bounds[1] + 1);
            nvgLineTo(mNVGContext, px - 7.f, bounds[1] + 1);
            nvgFill(mNVGContext);

            nvgFillColor(mNVGContext, Color(255, 255));
            nvgFontBlur(mNVGContext, 0.0f);
            nvgTextBox(mNVGContext, (float)(pos.x() - h), (float)pos.y(), (float)tooltipWidth,
...
void Screen::updateFocus(Widget *widget){
    for(auto w: mFocusPath) {
      if(!w || !w->parent() || w->getRefCount()<=0 || w->getRefCount()>10000 || !w->focused()) continue;            // sbassett - if the widget is deleted, can this be called with a nullptr?
